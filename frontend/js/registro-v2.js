...
window.RegistroV2 = {
    ...
    { id: 3, nombre: 'Implante Hormonal Subdérmico', categoria: 'Dispositivos' },
    { id: 4, nombre: 'Condón Masculino', categoria: 'Barrera' },
    { id: 5, nombre: 'Collar del Ciclo', categoria: 'Naturales' },
    { id: 6, nombre: 'Método de Lactancia y Amenorrea (MELA)', categoria: 'Naturales' },
    { id: 7, nombre: 'Anticoncepción Quirúrgica Voluntaria Femenina', categoria: 'Quirúrgica' },
    { id: 8, nombre: 'Anticoncepción Quirúrgica Voluntaria Masculina', categoria: 'Quirúrgica' },
    { id: 9, nombre: 'Dispositivo Intrauterino (DIU)', categoria: 'Dispositivos' },
...
formatearFechaParaDB(fechaInput) {
    ...
    async init() {
        ...
        try {
            ...
            this.showToast('Error al cargar el formulario: ' + error.message, 'error');
        }
    },
    ...
    actualizarHeaderAlcance(user) {
        ...
        if (user.rol === 'auxiliar_enfermeria') {
            ...
            if (user.territorios && user.territorios.length > 0) {
                const nombresTerritorios = user.territorios.map(t => t.nombre).join(', ');
                headerElement.textContent = nombresTerritorios;
            } else {
                headerElement.textContent = 'Asistente Técnico';
            }
        } else if (user.rol === 'coordinador_municipal' || user.rol === 'encargado_sr') {
    ...
    renderMetodos() {
        ...
        container.innerHTML = this.metodos.map(metodo => `
    ...
    async cargarTablaUsuarias() {
        ...
        try {
            ...
            if (usuarias.length > 0) {
                tbody.innerHTML = usuarias.map(u => `
                    <tr class="border-t border-gray-200 hover:bg-blue-50 transition">
                        <td class="px-4 py-3 text-gray-800">${u.nombres} ${u.apellidos}</td>
                        <td class="px-4 py-3 text-gray-600 text-sm">${u.comunidad || 'N/A'}</td>
                        <td class="px-4 py-3">
                            <span class="${this.getColorTipo(u.tipo_usuaria)} text-xs font-semibold">
                                ${this.formatTipoSimple(u.tipo_usuaria)}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-600 text-center">${u.total_visitas || 0}</td>
                        <td class="px-4 py-3 text-center">
                            <button 
                                onclick='RegistroV2.seleccionarDesdeTabla(${JSON.stringify(u).replace(/'/g, "&#39;")})'
    ...
    async buscarUsuaria() {
        ...
        try {
            ...
            if (error.message && error.message.includes('403')) {
                this.showToast('Usuaria encontrada pero no está en su alcance', 'error');
            } else {
    ...
    mostrarUsuariaEncontrada(data) {
        ...
        resultadoDiv.className = 'bg-green-50 border border-green-300 rounded-lg p-4';
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-sm text-gray-700 space-y-1">
                        <p><strong>Nombre:</strong> ${usuaria.nombres} ${usuaria.apellidos}</p>
                        <p><strong>DPI:</strong> ${usuaria.dpi}</p>
                        <p><strong>Comunidad:</strong> ${usuaria.comunidad_nombre || 'N/A'}</p>
                        <p><strong>Tipo:</strong> <span class="font-semibold ${this.getColorTipo(usuaria.tipo_usuaria)}">${this.formatTipo(usuaria.tipo_usuaria)}</span></p>
                        <p><strong>Total Visitas:</strong> ${usuaria.total_visitas || 0}</p>
                        ${usuaria.fecha_ultima_visita ? `<p><strong>Última Visita:</strong> ${this.formatFecha(usuaria.fecha_ultima_visita)}</p>` : ''}
                    </div>
                    <button 
    ...
    mostrarFormularioNuevaUsuaria(dpi) {
        const resultadoDiv = document.getElementById('resultado-busqueda');
        resultadoDiv.className = 'bg-yellow-50 border border-yellow-300 rounded-lg p-4';
        resultadoDiv.innerHTML = `
            <div class="flex items-start gap-3">
    ...
    async irPaso2(dpi) {
        if (!/^\d{13}$/.test(dpi)) {
            this.showToast('DPI inválido. Debe contener exactamente 13 dígitos', 'error');
            return;
        }
    ...
    // ===== CARGAR COMUNIDADES PERMITIDAS (CORREGIDO) =====
    async cargarComunidadesPermitidas() {
        ...
        try {
            ...
            // ✅ Usar nuevo endpoint que filtra por alcance
            const response = await SGPF.apiCall('/comunidades');
            
            if (!response.success || !response.data) {
                throw new Error('Error cargando comunidades');
            }
            
            const comunidades = response.data;
            
            if (comunidades.length === 0) {
                select.innerHTML = '<option value="">Sin comunidades disponibles</option>';
                this.showToast('No tiene comunidades asignadas para registro', 'warning');
                return;
            }
            
            select.innerHTML = comunidades.map(c =>
                `<option value="${c.id}">${c.nombre} (${c.codigo_comunidad})</option>`
            ).join('');
    ...
    const apellidos = document.getElementById('usuaria-apellidos')?.value.trim();
    ...
    if (!/^\d{13}$/.test(dpi)) {
        this.showToast('DPI inválido. Debe contener EXACTAMENTE 13 dígitos numéricos', 'error');
        document.getElementById('usuaria-dpi')?.focus();
        return;
    }
    ...
    continuarConUsuaria() {
        ...
        document.getElementById('visita-usuaria-nombre').textContent = `${usuaria.nombres} ${usuaria.apellidos}`;
        document.getElementById('visita-usuaria-dpi').textContent = usuaria.dpi;
        document.getElementById('visita-usuaria-comunidad').textContent = usuaria.comunidad_nombre || 'N/A';
        document.getElementById('visita-usuaria-tipo').textContent = this.formatTipo(usuaria.tipo_usuaria);
        document.getElementById('visita-usuaria-tipo').className = `font-semibold ${this.getColorTipo(usuaria.tipo_usuaria)}`;
        document.getElementById('visita-usuaria-total').textContent = usuaria.total_visitas || 0;
    },
    ...
    const observaciones = document.getElementById('observaciones-visita')?.value.trim();
    ...
    try {
        ...
        if (response.success) {
            ...
            // Esperar 1.5 segundos y volver al dashboard
            setTimeout(() => {
    ...
    actualizarIndicadores() {
        [1, 2, 3].forEach(paso => {
            ...
            if (paso <= this.state.pasoActual) {
                ...
                circle.className = 'w-8 h-8 rounded-full bg-gray-300 text-white flex items-center justify-center font-bold';
    ...
    mostrarHistorial(visitas) {
        ...
        content.innerHTML = visitas.map(v => `
            <div class="flex justify-between items-center py-2 border-b border-gray-200">
                <div>
                    <span class="font-medium text-gray-800">${v.metodo}</span>
                    <span class="text-xs text-gray-500 ml-2">${this.formatFecha(v.fecha_visita)}</span>
                </div>
