...
const SGPFApp = {
  ...
  async initializeInterface() {
    ...
    // ✨ NUEVO: Inicializar SecurityManager
    if (window.SecurityManager) {
      try {
        await window.SecurityManager.init();
        console.log("✅ SecurityManager inicializado desde app.js");
      } catch (error) {
        console.error("❌ Error inicializando SecurityManager:", error);
      }
    } else {
      console.warn("⚠️ SecurityManager no disponible");
    }
  },
  ...
  setupNavigation(user) {
    ...
    if (desktopNav) {
      const desktopContent =
        desktopNav.querySelector(".desktop-nav-content") || desktopNav;
    ...
    if (logoutBtn) {
      logoutBtn.addEventListener("click", async () => {
        if (confirm("¿Estás seguro de que quieres cerrar sesión?")) {
          // Destruir SecurityManager antes de logout
          if (window.SecurityManager) {
          ...
          if (window.SGPFConfig) {
            window.SGPFConfig.logSecurityEvent("manual_logout", {
